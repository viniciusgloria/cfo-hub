    ...
    <Modal isOpen={isOpen} onClose={() => { onClose(); }} title={tipo === 'ajuste' ? 'Solicitar Ajuste de Ponto' : 'Justificar Falta/Atraso'}>
      <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-3">
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800 shadow-sm">
          <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 mb-1"><Calendar size={16} className="text-emerald-500"/>Data</label>
          <input type="text" value={data} disabled className="modal-accent-field w-full mt-2" />
        </div>

        {tipo === 'ajuste' && (
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800 shadow-sm">
            <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 mb-1"><FileText size={16} className="text-emerald-500"/>Tipo de ajuste</label>
            ...
            <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 mb-1"><Clock size={16} className="text-emerald-500"/>Horário (HH:MM)</label>
            <input type="time" value={horario} onChange={(e) => setHorario(e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-lg" />
          </div>
        )}

        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800 shadow-sm">
          <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 mb-1"><Calendar size={16} className="text-emerald-500"/>Registros do dia</label>
          <div className="text-sm text-gray-600 mb-2 mt-2">
            {(registros.find(r => r.data === data)?.punches || []).map((p: any) => (
              <div key={p.ts} className="flex items-center justify-between py-1">
        ...
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800 shadow-sm">
          <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 mb-1"><FileText size={16} className="text-emerald-500"/>Anexos {tipo === 'ajuste' ? '(opcional)' : ''}</label>
          ...
          {attachments.length > 0 && (
            <div className="mt-2 space-y-2">
              ...
              <div className="text-xs text-gray-600">Total: {fmtSize(totalSize)}</div>
              {hasError && <div className="text-xs text-red-600">Remova anexos inválidos antes de prosseguir.</div>}
              {isUploading && <div className="text-xs text-gray-500">Enviando anexos...</div>}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <textarea value={motivo} onChange={(e) => setMotivo(e.target.value)} rows={4} placeholder={tipo === 'ajuste' ? 'Ex: Esqueci de registrar...' : 'Ex: Consulta médica...'} className="w-full px-3 py-2 border border-gray-200 rounded-lg resize-none" />
        </div>
