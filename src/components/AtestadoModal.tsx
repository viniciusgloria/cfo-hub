    ...
    <Modal isOpen={isOpen} onClose={() => { onClose(); }} title="Inclusão de Atestado Médico">
      <div className="space-y-3 p-4 sm:p-6">
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800 shadow-sm">
          ...
          <input type="text" value={data} disabled className="bg-white text-gray-900 border border-gray-200 rounded-lg px-3 py-2 w-full mt-2" />
        </div>

        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800 shadow-sm">
          <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 mb-1"><FileText size={16} className="text-emerald-500"/>Anexos (PDF ou imagens, até 3MB)</label>
          <Dropzone onFiles={handleFiles} />

          {attachments.length > 0 && (
            <div className="mt-2 space-y-2">
              {attachments.map((a) => (
                <div key={a.id} className="flex items-center justify-between gap-3 border rounded-md p-2 bg-gray-50 dark:bg-gray-900">
              ...
              <div className="text-xs text-gray-600">Total: {(totalSize / 1024 / 1024).toFixed(2)} MB</div>
        ...
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800 shadow-sm">
          <label className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 mb-1"><Info size={16} className="text-emerald-500"/>Observação</label>
          <textarea value={motivo} onChange={(e) => setMotivo(e.target.value)} rows={3} className="w-full px-3 py-2 rounded-lg resize-none bg-white text-gray-900 border border-gray-200 mt-2" placeholder="Ex: Retorno médico com atestado anexo." />
        </div>
