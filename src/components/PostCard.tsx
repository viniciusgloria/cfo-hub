        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            {post.attachments && post.attachments.length > 0 && (
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
                {post.attachments.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="relative overflow-hidden rounded border border-gray-200 bg-white">
                      {isImage ? (
                        <button
                          type="button"
                        ...
                        <div className="p-3">
                          <p className="text-sm font-medium text-gray-700 truncate" title={a.name}>{a.name}</p>
                          <a href={a.dataUrl} download={a.name} className="text-xs text-blue-600 underline mt-1 inline-block">
                            baixar
                          </a>
                        </div>
                      )}
                      <div className="px-3 pb-2">
                        <p className="text-[10px] text-gray-400 break-words">{a.remoteUrl}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <div className="flex-1" />
              <div className="text-xs text-gray-500">{post.comments.length} comentarios</div>
            </div>
